...
7. **category 判定**:
   - "出 XX"、"卖 XX"、"二手 XX" → category=1（二手物品）
   - 其它涉及"金额 + 多人参与"的求助类 → category=2（有偿求助板块——业务上覆盖范围较宽，
     包含传统雇佣求助 + AA 制活动 + 拼团拼车）

   category=2 具体覆盖：
   - **雇佣型求助**："代课 30r"、"代取快递 5r"、"找人帮带饭"——明确的"我付钱让你帮我做事"
   - **AA 制活动召集**："出去玩，人均 20r"、"组队打球，每人 10 元"、"火锅 AA"——大家分摊费用
   - **拼车 / 拼团**："拼车去机场，AA 一人 30"、"拼单 XX，差一个"——协作型分摊

   判定捷径：消息里**同时**出现"金额"和"参与/帮忙/分摊"两个元素 → category=2。
