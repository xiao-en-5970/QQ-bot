...
5. **图片归属 / 图文交杂多商品**（输入是用户在 60s 内的连续发言整段）:
   ...
   **配对策略**：按"出现顺序号"对齐——商品文字按出现顺序编号 t1..tN，图片按
   ...
   先看聊天记录是哪一种结构——决定走哪条规则：

   #### A. 散乱独立上架（每条记录是自包含的"图片+文字"完整商品）

   每条记录的 segments 里**同时含**图片占位和商品文字+价格（即"图文同条"），
   或者每条记录都自带至少 1 张图。例如：
   ...
   这是 A 类——3 条记录是 3 个独立商品。规则：

   1. 每条来自聊天记录的消息，单独就是一次独立的发布行为；
   2. 不要跨条合并 / 不要把别的商品混进来；
   3. image_message_ids / source_message_ids 只填**当前这条消息的 message_id**。

   #### B. 批量图文一对一上架（图条 / 文条 交替穿插）

   每条记录**单独**不构成完整商品——比如某条 segments 只有 [图片] 没文字、
   下一条只有商品文字没图。整段聊天记录其实是用户某时刻"批量出闲置"时一条条快速
   连发的，图文按出现顺序对齐。例如：

     消息1（01:15:26）：[来自聊天记录] [图片]
     消息2（01:15:26）：[来自聊天记录] 不规则大花瓶，一个4块钱
     消息3（01:15:26）：[来自聊天记录] [图片]
     消息4（01:15:26）：[来自聊天记录] 小喷壶，1块钱
     ...
     消息N（01:15:26）：[来自聊天记录] 收纳盒 1元
     消息N+1（01:15:26）：[来自聊天记录] [图片]
     消息N+2（01:15:26）：[来自聊天记录] [图片]
     消息N+3（01:15:26）：[来自聊天记录] 狼人杀卡牌 2元
     ...

   这是 B 类——**不要**把每条记录当独立判定（那会让所有商品 image_message_ids
   都是空）。正确做法：**完全套用前面"批量图文一一对应纠错"那节的规则**，把整段
   聊天记录当成一个"批量上架"窗口，按"商品文字出现顺序 ti ↔ 图片出现顺序 pi"
   跨条配对。每个 publish_good 的 image_message_ids 填**对应那张图所在记录的
   message_id**（不是商品文字那条的 message_id）；source_message_ids 同时含
   商品文字那条 + 图片那条。

   #### 如何区分 A vs B

   - **A 类信号**：大多数记录的 segments 里**自身就含商品文字 + 图片**（图文同条），
     或者每条记录至少有图。
   - **B 类信号**：相当数量的记录"只有图、没文"或"只有文、没图"，且窗口内
     商品文字数 N ≥ 5、图片数 M 与 N 接近（|M-N| ≤ 2），消息时间几乎相同。
   - 介于两者之间时按 B 处理（图文配对更接近用户本意）。

   #### 反例（**不要**这样判）

   - A 类场景把 3 条合并成 1 个商品 → 丢失另外两个商品。
   - B 类场景按 A 处理，每条独立判定 → 所有 publish_good 的 image_message_ids 都是空。
   - 不论 A 还是 B：**绝对不要**把**非聊天记录**消息的 message_id 混入聊天记录商品的引用里。
