...
type RecognizeAction struct {
	...
	// IsBatch 标记本商品是"合并聊天记录批量上架"——前端显示"批量上架"tag、
	// 价格统一面议、title 是多个商品名用逗号串联。只在聊天记录展开路径置 true。
...
5. **图片归属 / 图文交杂多商品**（输入是用户在 60s 内的连续发言整段）:
   ...
   **配对策略**：按"出现顺序号"对齐——商品文字按出现顺序编号 t1..tN，图片按
   ...
   每条这种消息的 segments **以 [来自聊天记录] 标记开头**。

   策略很简单：**整段聊天记录里所有带 [来自聊天记录] 标记的消息合并成 1 个
   publish_good**（"批量上架"）。不要尝试拆分成多个独立商品。

   具体规则：

   1. **只输出 1 个 publish_good**：所有带 [来自聊天记录] 的消息（不论图条 / 文条）
      合并；非聊天记录消息保持各自独立判定。
   2. **title**：把每条 [来自聊天记录] 消息里识别到的商品名用**中文逗号**串联，
      如 "不规则大花瓶，小喷壶，两用雨衣，防撞条，防尘膜，粉色衣服收纳盒，狼人杀卡牌，
      可撕拉指甲油，得力热熔枪，闲置大相册"。
      - 如果某条记录里识别不出明确商品名（只有补充描述），就跳过它不入 title。
      - title 不要带价格、数量、"出"、"卖"等动词，**只**保留物品名本身。
   3. **description**：把每条记录里的**原始文字**按出现顺序拼起来（句间用顿号 "、"
      或换行分隔即可），便于用户在 app 查看完整明细，包括每件物品的具体价格。
   4. **price / negotiable**：**统一面议**——price 字段**不输出**，negotiable=true。
      （即使每条记录里有明确数字价，整体合并后也按面议，避免把多种价格搅在一起。）
   5. **category**：按整体语义判，绝大多数批量上架都是 category=1（二手）。如果
      整段明显是求购（含"求"、"收"等关键字），才用 category=2。
   6. **stock**：不输出（默认 1）。
   7. **image_message_ids**：含**所有**带 [来自聊天记录] 的图片消息的 message_id
      （不论它对应哪个商品名，整批图都属于这个合并商品）。
   8. **source_message_ids**：含**所有**带 [来自聊天记录] 的消息 message_id
      （图条 + 文条都算）。
   9. **is_batch**：**必填 true**——这是给 app 前端展示"批量上架"tag 用的；不带
      [来自聊天记录] 标记的普通商品 is_batch 不填或填 false。

   典型例子（输入是用户转发的合并聊天记录，bot 已展开成 20 条带标记的消息）：

     消息1：[来自聊天记录] [图片]
     消息2：[来自聊天记录] 不规则大花瓶，一个4块钱
     消息3：[来自聊天记录] [图片]
     消息4：[来自聊天记录] 小喷壶，1块钱
     ...（共 20 条，9 个商品 + 10 张图）...

   **正确输出**（**只** 1 个 publish_good）：
     {
       "type": "publish_good",
       "confidence": 1.0,
       "reason": "整段合并聊天记录批量上架，按整段合并为单一批量商品",
       "title": "不规则大花瓶，小喷壶，两用雨衣，防撞条，防尘膜，粉色衣服收纳盒，狼人杀卡牌，可撕拉指甲油，得力热熔枪",
       "description": "不规则大花瓶，一个4块钱、小喷壶，1块钱、两用雨衣，3块钱、防撞条全新两米长，2块钱、防尘膜全新一卷，2块钱、粉色衣服收纳盒，表面有点脏挺大的，1块钱、狼人杀卡牌，2块钱、可撕拉指甲油，用过一次，两瓶3块钱、得力热熔枪，送胶棒，10块钱",
       "negotiable": true,
       "category": 1,
       "is_batch": true,
       "image_message_ids": [<10 张图的 msgid 全部列出>],
       "source_message_ids": [<20 条聊天记录消息的 msgid 全部列出>]
     }

   **反例**（不要这样判）：
   - 不要拆成 9 个独立 publish_good——批量场景下用户感知就是"我转发了一份清单"。
   - 不要给批量商品填 price 字段——多个不同价混在一起没意义。
   - 不要把**非聊天记录**消息的 message_id 混入这个合并商品的 image/source_message_ids。
   - 不要漏 is_batch:true——app 前端要靠它显示"批量上架"tag。
