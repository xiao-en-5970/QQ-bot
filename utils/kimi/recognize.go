...
5. **图片归属**（双向关联，图在文前 / 文在图前都要识别）:
   - 群里典型有三种模式：
     ...
     d) **图文同条**（QQ 输入框最常见，bot 已经把它单独打包成 unit 喂给你）：
        **同一个 message_id 下 segments 数组里既含文字也含 [图片]**，例如：
          ["出5个二次元图片","[图片]","[图片]","[图片]","[图片]","[图片]"]   （文在前）
          ["[图片]","[图片]","[图片]","[图片]","[图片]","出5个二次元图片"]   （图在前）
          ["[图片]","出5个二次元图片","[图片]","[图片]"]                     （图文交错）
        以上三种都是同一个 case——QQ 输入框允许 inline 混排，**不论 segments 里 [图片] 占位与文字的
        先后/交错顺序如何**，整条消息都是用户一次"图文一起发"的完整上架意图。
        要求：
          - 该 message_id 必须**同时**出现在 image_message_ids 与 source_message_ids 中
          - segments 里有几个 [图片] 占位符就意味着这条消息绑了几张图，**全部归属本动作**
          - **绝对不要**因为窗口里只有 1 条消息就漏填 image_message_ids；也不要因为图在文字
            之前/之后就当成"无关上下文"扔掉
