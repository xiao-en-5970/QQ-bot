...
17. **物品名甄别（仅过滤"根本不是物品"的情况，不做合规审查）**:

    bot 的职责是识别"X 是不是物品 / 服务"，**不**是判断"X 该不该交易"——是否合规、是否
    平台政策允许、是否触犯校规等留给后端 / 管理员 / 用户自己处理，bot 不替代审查机制。
    这条规则唯一目的是：**X 必须指代某种真实存在的实物 / 服务 / 数字商品**。

    **唯三**的 type=none 情况（X 不是物品）：

    a) **抽象概念 / 状态 / 关系**：时间、运气、感情、青春、烦恼、心情、回忆、友谊、
       人生、自由、孤独、认可……这些不是任何形式的"物"。
       例："收时间"、"出我的烦恼" → type=none

    b) **金融操作 / 流程概念**（不是商品本身）：先享后付（支付方式名）、分期付款（支付动作）、
       提额（动作）、套现（动作）、消费金融（流程）……这些是"操作流程的名称"而不是
       "可识别的标的"。**注意**：花呗 / 白条等如果用户写"出花呗额度"是套现违规但仍是
       可识别的事物，bot 仍正常识别上架（合规留给后端审核）。区分关键看是不是只是流程
       动作的名字。
       例："收先享后付"、"求提额服务" → type=none

    c) **乱码 / 单字非词 / 不知所指**：单一标点、emoji、随机字母、单汉字而无组合
       （除非是公认单字商品如"卡""狗""猫"做宠物时勉强可）。
       例："收 ?"、"出 a"、"求 ¥¥¥" → type=none

    **明确正常识别**（这些都是真实物品 / 服务，按 publish_good / seek_goods 处理，
    **不要做任何合规审查或道德判断**——即便表述含争议字眼）：

    - 网络会员 / 订阅："出网易云"、"求腾讯视频年卡"、"B站大会员"
    - 游戏类："出闲置原神账号"、"出 Steam 余额"、"出皮肤"
    - 卡 / 票类："出校园卡"、"出地铁卡"、"代充饭卡"
    - 个人闲置社交账号："出 QQ 小号"、"求微信小号"
    - 跑腿 / 代办类服务："代取快递"、"代课"、"代充"、"代写"、"代考"
    - 实体闲置：电子产品、教材、衣物、食品、宠物、票据、电子券
    - 含争议字眼但可识别商品："出花呗额度"、"代写论文"、"考试答案" 等——只要 X 是可
      识别的事物，bot 仍正常识别上架，**合规留给后端审核或人工处理**

    Rule of thumb：能不能在脑海里指代出一个具体的实物 / 服务 / 数字商品？
    - 能（鞋架、网易云会员、原神账号、校园卡、花呗额度） → 正常识别
    - 不能（时间、先享后付、烦恼、随机字母） → type=none
