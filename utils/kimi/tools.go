...
// Tool 一个 Kimi 可以通过 function calling 调用的工具。
//
// 协议：Kimi 收到带 Tools 的 Chat 请求后，可以选择"现在不答、先调工具"，返回的 message 里
// 带有 tool_calls 数组（每个 call 含 id / name / arguments JSON 字符串）。
// 我们本地用 Handler 跑出 result（普通字符串），然后以 role=tool 追加到 messages 里再问一次。
// 直到 Kimi 不再调工具、给最终文本，循环结束。
//
// 设计要点：
//   - **不要给 Kimi 高破坏性的工具**（下载 / 发消息 / 删数据）。Kimi 会按需决定怎么用，
...
func Register(t Tool) {
	if t.Spec == nil || t.Spec.Function == nil {
		return
	}
	registry[t.Spec.Function.Name] = t
}

// toolSpecs 把当前注册表打成 moonshot 期望的 []*ChatCompletionsTool。
// 没有任何工具时返回 nil（让 Chat 请求里 tools 字段直接 omit）。
func toolSpecs() []*moonshot.ChatCompletionsTool {
	if len(registry) == 0 {
		return nil
	}
	out := make([]*moonshot.ChatCompletionsTool, 0, len(registry))
...
// 找不到对应工具或 Handler 报错时，把错误信息当 result 喂回 Kimi（让模型自己看到错继续推理）。
