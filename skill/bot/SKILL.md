...
- 模糊判定一律 drop（不打扰用户）
- 真触发上架时**必须 @ 用户确认**（"@用户 上架成功 二手「三层鞋架」 6 元"）
- **不需要管理员预览**——目标是"用户群里发一句无感同步到 app"
- 模糊时优先判**不上架**（保守）
- 跨多条消息要按"窗口聚合"处理（详见 `recognition.md`）
- 回执文案**站在不懂代码的用户视角**——不暴露 `goods_id`/`article_id` 等专业字段，详见 `verbosity.md` 的"文案原则"段
...
| `reply_in_group(group_id, at_qq, text)` | bot 在群里 @ 用户回执（"上架成功 二手「XXX」 6 元"） |
...
6. **回执友好**：业务真触发后，调 `reply_in_group` @ 用户简短确认（"上架成功 二手「三层鞋架」 6 元"），不要长篇大论；**不要暴露** `goods_id`/`article_id` 这类专业字段——用户看不懂、也不需要（他在 app 里能看到自己刚发的）。
